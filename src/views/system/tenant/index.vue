<template>
  <div class="page-container">
    <el-card class="page-search" shadow="never">
      <el-form ref="queryFormRef" :model="tableData.params" :inline="true">
        <el-form-item prop="keywords" label="关键字">
          <el-input v-model="tableData.params.keywords" placeholder="租户名称/租户编码/域名" clearable @keyup.enter="handleQuery" />
        </el-form-item>
    ...
    <el-card class="page-content" shadow="never">
      <div class="page-toolbar">
        <div class="page-toolbar__left">
          <el-button v-hasPerm="['sys:tenant:create']" type="success" icon="plus" @click="openDialog()">
            新增
          </el-button>
          <el-button v-hasPerm="['sys:tenant:delete']" type="danger" icon="delete" :disabled="ids.length === 0" @click="handleDelete()">
            删除
          </el-button>
        </div>
      </div>

      <el-table v-loading="loading" :data="tableData.list" highlight-current-row border @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" align="center" :selectable="isTenantSelectable" />
        ...
        <el-table-column fixed="right" label="操作" width="320">
          <template #default="scope">
            <el-tooltip v-if="!isPlatformTenantId(scope.row.id)" content="更换租户套餐（将影响可用功能）" placement="top">
              <el-button v-hasPerm="['sys:tenant:plan-assign']" type="primary" size="small" link icon="menu" title="更换租户套餐（将影响可用功能）"
                @click="openTenantPlanDialog(scope.row)">
                更换套餐
              </el-button>
            </el-tooltip>
            <el-tooltip v-if="!isPlatformTenantId(scope.row.id)" content="在当前套餐范围内配置租户可用功能" placement="top">
              <el-button v-hasPerm="['sys:tenant:plan-assign']" type="primary" size="small" link icon="setting" :disabled="!scope.row.planId"
                title="在当前套餐范围内配置租户可用功能" @click="openTenantCustomizeDialog(scope.row)">
                套餐功能配置
              </el-button>
            </el-tooltip>
            <el-button v-hasPerm="['sys:tenant:update']" type="primary" size="small" link icon="edit" @click="openDialog(scope.row.id)">
              编辑
            </el-button>
            <el-button v-if="!isPlatformTenantId(scope.row.id)" v-hasPerm="['sys:tenant:delete']" type="danger" size="small" link icon="delete"
    ...
    <el-dialog v-model="dialogState.visible" :title="dialogState.title" width="600px" @close="closeDialog">
      <el-form ref="dataFormRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <el-form-item label="租户编码" prop="code">
          <el-input v-model="formData.code" placeholder="请输入租户编码" :disabled="formData.id != null && String(formData.id) !== ''" />
        </el-form-item>
        ...
        <el-form-item v-if="!isPlatformTenant && (formData.id == null || String(formData.id) === '')" label="租户套餐" prop="planId">
        ...
        <el-form-item label="过期时间" prop="expireTime">
          <el-date-picker v-model="formData.expireTime" type="datetime" placeholder="不填表示永不过期" value-format="YYYY-MM-DD HH:mm:ss"
            style="width: 100%" />
        </el-form-item>

        <el-form-item v-if="formData.id == null || String(formData.id) === ''" label="管理员账号" prop="adminUsername">
          <el-input v-model="formData.adminUsername" placeholder="为空则系统生成" />
        </el-form-item>

        <el-form-item v-if="formData.id != null && String(formData.id) !== ''" label="状态" prop="status">
    ...
    <el-dialog v-model="tenantPlanSelectVisible" title="更换租户套餐" width="520px" @close="closeTenantPlanSelectDialog()">
      <el-form label-width="90px" class="mb-3">
        ...
        <el-form-item label="目标套餐">
          <el-select v-model="tenantPlanId" placeholder="请选择租户套餐" style="width: 100%" @change="handlePlanChange">
            <el-option v-for="item in planOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
      </el-form>
      ...
      <div class="mb-3">
        ...
        <el-alert type="warning" :closable="false" show-icon title="⚠ 降级套餐将导致超出套餐范围的功能被移除，相关功能将立即不可用。" />
      </div>

      <template #footer>
        <div class="dialog-footer">
          <el-button v-hasPerm="['sys:tenant:plan-assign']" type="primary" :disabled="!tenantPlanId || isPlanMenuEmpty"
    ...
    <el-drawer v-model="tenantPlanDialogVisible" title="套餐功能配置" size="640px" @close="closeTenantPlanDialog()">
      ...
      <el-scrollbar class="h-[60vh]">
        <el-tree ref="menuTreeRef" node-key="value" show-checkbox :props="menuTreeProps" :data="menuPermOptions"
          :default-expanded-keys="menuExpandedKeys" :default-expand-all="false" class="mt-5" @check="handleMenuCheckedChange">
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button v-hasPerm="['sys:tenant:plan-assign']" type="primary" :disabled="isPlanMenuEmpty" @click="handleTenantPlanSubmit">
            保存配置
          </el-button>
...
<script setup lang="ts">
  ...
  const tableData = reactive<PageResult<TenantItem>>({
    list: [],
    total: 0,
    params: { //查询参数
      pageNum: 1,
  ...
  function fetchData(): void {
    loading.value = true;
    TenantAPI.getPage(tableData.params)
      .then((data) => {
        tableData.list = data.list.map((item: TenantItem) => ({
          ...item,
